...
// Memastikan semua variabel yang akan di-log bernilai non-NULL
$id_login = $_SESSION['id_pengguna'] ?? 0;
$username_login = $_SESSION['username'] ?? 'SYSTEM_USER'; // FIX: Nilai non-NULL dijamin
...
$conn->begin_transaction(); // Mulai transaksi untuk keamanan
...
try {
    ...
    if ($conn->query($sql_delete) === TRUE) {
        ...
        throw new Exception("Akun masih terhubung ke tabel master/transaksi lain.");
    }
    ...
    $_SESSION['error_delete'] = "❌ Error saat menghapus akun: {$error_detail}";
    
    // LOG AKTIVITAS: Penghapusan Gagal karena FK/Rollback
    if (function_exists('logAktivitas')) {
